...
const TARGET_BOL_NUMBER = 'HLCUSHA2307ADRIA';
...
async function cleanupBolNumber() {
  ...
  try {
    ...
    console.log(`Found ${bolDocuments.length} documents with BOL number ${TARGET_BOL_NUMBER}`);
    
    if (bolDocuments.length === 0) {
      console.log('No documents found to clean up.');
      
      // Also check for documents where the bolNumber might be in a different field structure
      const alternativeCheck = await documents.find({
        $or: [
          { bolNumber: TARGET_BOL_NUMBER },
          { 'document.bolNumber': TARGET_BOL_NUMBER },
          { 'document.shipmentDetails.bolNumber': TARGET_BOL_NUMBER }
        ]
      }).toArray();
      
      if (alternativeCheck.length > 0) {
        console.log(`Found ${alternativeCheck.length} documents using alternative field structure`);
        bolDocuments.push(...alternativeCheck);
      } else {
        // Check if the BOL number exists as a substring in any document's JSON
        const regexCheck = await documents.find({
          $where: `function() {
            return JSON.stringify(this).indexOf("${TARGET_BOL_NUMBER}") !== -1;
          }`
        }).toArray();
        
        if (regexCheck.length > 0) {
          console.log(`Found ${regexCheck.length} documents containing the BOL number as a substring`);
          bolDocuments.push(...regexCheck);
        } else {
          return;
        }
      }
    ...
    // 7. Also check and clean any files that might have the BOL number in metadata
    const filesWithBolNumber = await files.find({
      $or: [
        { 'metadata.bolNumber': TARGET_BOL_NUMBER },
        { filename: { $regex: TARGET_BOL_NUMBER } }
      ]
    }).toArray();
    
    console.log(`Found ${filesWithBolNumber.length} files with BOL number in metadata or filename`);
    
    for (const file of filesWithBolNumber) {
      try {
        await bucket.delete(file._id);
        console.log(`Deleted file with BOL reference: ${file.filename} (${file._id})`);
      } catch (err) {
        console.error(`Error deleting file ${file._id}:`, err.message);
      }
    }
